...
function StartHeliCamera()
	...
	if
		inHeli
		and camHelis[GetEntityModel(inHeli)]
		and IsHeliHighEnough(inHeli)
		and GetPedInVehicleSeat(inHeli, 0) == LocalPlayer.state.ped
		...
		Citizen.CreateThread(function()
			while heliCamera do
				...
				if locked_on_vehicle then
					if
						not DoesEntityExist(locked_on_vehicle)
						or not HasEntityClearLosToEntity(inHeli, locked_on_vehicle, 4294967295)
						or #(GetEntityCoords(LocalPlayer.state.ped) - GetEntityCoords(locked_on_vehicle)) > 300.0
					then
				...
				if
					GetVehiclePedIsIn(LocalPlayer.state.ped) ~= inHeli
					or not IsHeliHighEnough(inHeli)
					or LocalPlayer.state.isDead
					or GetPedInVehicleSeat(inHeli, 0) ~= LocalPlayer.state.ped
				then
					heliCamera = false
				end
			end
...
function HeliRappel()
	inHeli = GetVehiclePedIsIn(LocalPlayer.state.ped, false)
	if inHeli and camHelis[GetEntityModel(inHeli)] then
		if
			IsHeliHighEnough(inHeli)
			and (GetPedInVehicleSeat(inHeli, 1) == LocalPlayer.state.ped or GetPedInVehicleSeat(inHeli, 2) == LocalPlayer.state.ped)
			and not LocalPlayer.state.isDead
		then
			if _hasRappel == false then
				_hasRappel = true
				Notification:Info("Prepping Rappel")
				PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
				TaskRappelFromHeli(LocalPlayer.state.ped, 1)
				Wait(10000)
				_hasRappel = false
			else
				Notification:Error("Rappel is already being initiated!")
			end
		else
			Notification:Info("Cannot rappel from this seat")

...
function RunPlacementThread(model, canPlaceInside, useGizmo, showFurnitureButtons, startCoords, startHeading, startRotation, maxDistOverride)
    loadModel(model)
    _maxDist = 5.0
    _gizmoCam = false
    ...
    if not useGizmo then
        Citizen.CreateThread(function()
            while _placeData ~= nil and _placing do
                ...
                isValid = hit and (not IsEntityAVehicle(entity) and not IsEntityAPed(entity)) and #(coords - myPos) <= _maxDist and (canPlaceInside or (GetInteriorFromEntity(myPed) == 0 and GetInteriorFromEntity(obj) == 0))
        ...
        EnterCursorMode()
        exports["method-objects"]:prepareGizmo(obj)
        ...
        Citizen.CreateThread(function()
            while _placeData ~= nil and _placing do
                ...
                if not _gizmoCam then
                    exports["method-objects"]:drawGizmo(obj)
                end

                isValid = #(GetEntityCoords(obj) - myPos) <= _maxDist and (canPlaceInside or (GetInteriorFromEntity(myPed) == 0 and GetInteriorFromEntity(obj) == 0))

...
COMPONENTS.Punishment.Unban = {
	...
	AccountID = function(self, aId, issuer)
		if COMPONENTS.Punishment:CheckBan("account", aId) then
			...
			if tPlayer == nil then
				tPlayer = COMPONENTS.Fetch:Offline("account", aId)
				dbf = true
			end
	...
	Identifier = function(self, identifier, issuer)
		if COMPONENTS.Punishment:CheckBan("identifier", identifier) then
			...
			local dbf = false
			if tPlayer == nil then
				tPlayer = COMPONENTS.Fetch:Offline("identifier", identifier)
				dbf = true
			end
			local iPlayer = COMPONENTS.Fetch:Source(issuer)
            local p = promise.new()
...
COMPONENTS.Punishment.Ban = {
	...
	AccountID = function(self, aId, expires, reason, issuer)
		...
		local dbf = false
		if tPlayer == nil then
			tPlayer = COMPONENTS.Fetch:Offline("account", tonumber(aId))
            print(tPlayer, tPlayer:GetData("Name"))
			dbf = true
		end
			...
			if tSource ~= nil then
				if expires ~= -1 then
					DropPlayer(
						tSource,
						string.format(
							"You're Banned, Appeal At https://discord.gg/quadrantrp \n\nReason: %s\nExpires: %s\nID: %s",
							reason,
							expStr,
							results._id
						)
					...
					DropPlayer(
						tSource,
						string.format(
							"You're Permanently Banned, Appeal At https://discord.gg/quadrantrp \n\nReason: %s\nID: %s",
							reason,
							results._id
						)
					)

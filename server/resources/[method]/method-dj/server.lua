...
AddEventHandler("Core:Shared:Ready", function()
	exports["method-base"]:RequestDependencies("Animations", {
		"Callbacks",
	}, function(error)
		if #error > 0 then
			return
...
function RegisterCallbacks()
	Callbacks:RegisterServerCallback("method-dj:server:songInfo", function(source, data, cb)
		cb(previousSongs)
	end)

	Callbacks:RegisterServerCallback("method-dj:server:playMusic", function(source, data, cb)
		local src = source
	...
	Callbacks:RegisterServerCallback("method-dj:server:changeVolume", function(source, data, cb)
		local src = source
		local zoneNum = data.zoneNum
	...
	Callbacks:RegisterServerCallback("method-dj:server:stopMusic", function(source, data, cb)
		local src = source
		...
		TriggerClientEvent("method-dj:client:playMusic", src, { zone = data.zoneNum })
	end)

	Callbacks:RegisterServerCallback("method-dj:server:pauseMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
		if Config.Locations[data.zoneNum].playing then
			Config.Locations[data.zoneNum].playing = nil
	...
	Callbacks:RegisterServerCallback("method-dj:server:resumeMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
...
-- -- This is for that
-- RegisterNetEvent("method-dj:server:DestoryAll", function()
...
-- 	TriggerEvent("method-dj:server:DestoryAll")
-- end)

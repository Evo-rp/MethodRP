...
function OpenVehicleCustoms(canInstallPerformance, costMultiplier, settings)
	...
	if not settings.GOVERNMENT_PAINT then
		...
		customsMenuSubs["paint"].Add:Text(
			"Changing Either Primary Paint Option Costs $" .. math.ceil(_customsConfig.paintCost * costMultiplier),
		...
		customsMenuSubs["paint"].Add:Text(
			"Changing Either Secondary Paint Option Costs $" .. math.ceil(_customsConfig.paintCost * costMultiplier),
			{ "pad", "code", "center", "textLarge" }
		)
		...
		local _curChameleon = 0
		if
			type(originalData.chameleonColor) == "number"
			and originalData.chameleonColor
			and originalData.chameleonColor >= 223
		then
			_curChameleon = originalData.chameleonColor
		end
		customsMenuSubs["paint"].Add:Select(
			"Chameleon Color - $" .. math.ceil(_customsConfig.cost.chameleonColor * costMultiplier),
			{
				disabled = false,
				current = _curChameleon,
				list = _customsConfig.chameleons,
			},
			function(data)
				if data.data.value == originalData.chameleonColor then
					changingData.chameleonColor = nil
				else
					changingData.chameleonColor = data.data.value
				end

				if changingData.chameleonColor == 0 then
					changingData.paintType = { 1, 1 }
				end
		...
		customsMenuSubs["paint"].Add:Select(
			"Pearlescent Color - $" .. math.ceil(_customsConfig.cost.pearlescentColor * costMultiplier),
			{
				disabled = false,
				current = (
					(changingData.pearlescentColor ~= nil) and changingData.pearlescentColor
					or (originalData.pearlescentColor or 0)
				),
		...
		customsMenuSubs["paint"].Add:Select(
			"Interior Color - $" .. math.ceil(_customsConfig.cost.interiorColor * costMultiplier),
			{
				disabled = false,
				current = (
					(changingData.interiorColor ~= nil) and changingData.interiorColor
					or (originalData.interiorColor or 0)
				),
				list = _customsConfig.pearlescents,
			},
			function(data)
		...
		customsMenuSubs["paint"].Add:Select(
			"Dashboard Color - $" .. math.ceil(_customsConfig.cost.dashboardColor * costMultiplier),
			{
				disabled = false,
				current = (
					(changingData.dashboardColor ~= nil) and changingData.dashboardColor
					or (originalData.dashboardColor or 0)
				),
				list = _customsConfig.pearlescents,
			},
			function(data)
				if data.data.value == originalData.dashboardColor then
					changingData.dashboardColor = nil
				else
					changingData.dashboardColor = data.data.value
				end
		...
		customsMenuSubs["paint"].Add:Select(
			"Paint Presets - $" .. math.ceil(_customsConfig.paintCost * costMultiplier),
			{
				disabled = false,
				current = false,
				list = {
	...
	customsMenuSubs["extras"].Add:Text(
		"All Changes to Extras Cost $" .. math.ceil(_customsConfig.cost.extras * costMultiplier),
		{ "pad", "code", "center", "textLarge" }
	)
	...
	for i = 0, 12 do
		if DoesExtraExist(CUST_VEH, i) then
			...
			extraItemIds[extraString] = customsMenuSubs["extras"].Add:Button(
				"Extra #" .. extraString,
				{ success = originalData.extras[extraString] },
				function(data)
					if not changingData.extras then
						changingData.extras = {}
					end
	...
	customsMenuSubs["lighting"].Add:CheckBox(
		"Xenon Lights - $" .. math.ceil(_customsConfig.cost.mods.xenon * costMultiplier),
		{
	...
	customsMenuSubs["lighting"].Add:Select(
		"Xenon Lights Color - $" .. math.ceil(_customsConfig.cost.mods.xenonColor * costMultiplier),
		{
			disabled = false,
	...
	customsMenuSubs["lighting"].Add:Select(
		"Neons/Underglow - $" .. math.ceil(_customsConfig.cost.neonEnabled * costMultiplier),
		{
			disabled = false,
			current = 0,
	...
	for k, v in pairs(_customsConfig.wheelTypes) do
		if v.value ~= 6 or (v.value == 6 and DRIVING_VEHICLE_CLASS == 8) then
	...
	customsMenuSubs["wheels"].Add:Text("Custom Wheels", { "heading" })

	customsMenuSubs["wheels"].Add:CheckBox(
		"Enable or Disable Custom Wheels - $" .. math.ceil(_customsConfig.cost.customWheels * costMultiplier),
		{
			selected = originalData.mods.customTiresF,
			selected = originalData.mods.customTiresR,
		},
		function(data)
			if data.data.selected == originalData.mods.customTiresF then
				changingData.mods.customTiresF = nil
			else
				changingData.mods.customTiresF = data.data.selected
			end
			if data.data.selected == originalData.mods.customTiresR then
				changingData.mods.customTiresR = nil
			else
				changingData.mods.customTiresR = data.data.selected
			end
	...
	if not settings.BLOCK_TIRESMOKE then
		...
		customsMenuSubs["wheels"].Add:CheckBox(
			"Custom Tire Smoke - $" .. math.ceil(_customsConfig.cost.tyreSmoke * costMultiplier),
			{
		...
		customsMenuSubs["wheels"].Add:Text(
			"Tire Smoke Color - $" .. math.ceil(_customsConfig.cost.tyreSmokeColor * costMultiplier),
			{ "pad", "code", "center", "textLarge" }
		)
	...
	if canInstallPerformance then
		...
		for k, v in ipairs(_customsConfig.performanceMods) do
			...
			for i = 0, amount do
				...
				local price = math.ceil(
					(
						_customsConfig.cost.mods[v.mod][i + 1]
						or _customsConfig.cost.mods[v.mod][#_customsConfig.cost.mods[v.mod]]
					) * costMultiplier
				)
				table.insert(
					levelList,
					{ label = (modName and label or (v.name .. " Level " .. i + 1)) .. " - $" .. price, value = i }
				)
			end
		...
		customsMenuSubs["performance"].Add:CheckBox(
			"Turbo - $" .. math.ceil(_customsConfig.cost.mods.turbo * costMultiplier),
			{

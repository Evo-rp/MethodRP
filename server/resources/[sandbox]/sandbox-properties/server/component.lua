		...
		Buy = function(self, id, owner, payment)
			...
			Database.Game:updateOne({
				...
				if success then
					...
					if GlobalState[string.format("Char:Properties:%s", owner.Char)] ~= nil then
						local t = GlobalState[string.format("Char:Properties:%s", owner.Char)]
						table.insert(t, propertyId)
						GlobalState[string.format("Char:Properties:%s", owner.Char)] = t
					else
						GlobalState[string.format("Char:Properties:%s", owner.Char)] = {
							propertyId,
						}
					end

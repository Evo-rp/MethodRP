function RegisterChatCommands()
	...
	Chat:RegisterAdminCommand("showallprops", function(source, args, rawCommand)
		local pId = args[1]
		if not _showAllProps then
			_showAllProps = true
			TriggerClientEvent("Properties:Client:ShowAllPropsAdmin", source, true)
			Chat.Send.System:Single(source, "Show All Properties: Enabled")
		else
			_showAllProps = false
			TriggerClientEvent("Properties:Client:ShowAllPropsAdmin", source, false)
			Chat.Send.System:Single(source, "Show All Properties: Disabled")
		end
	end, {

...
RegisterNetEvent("Properties:Client:ShowAllPropsAdmin", function(show)
	if show then
		for k, v in pairs(_properties) do
			...
			if v.sold then
				...
				SetBlipSprite(HouseBlip, 414)
				SetBlipColour(HouseBlip, 39)
				SetBlipScale(HouseBlip, 0.45)
				SetBlipAsShortRange(HouseBlip, true)
				SetBlipCategory(HouseBlip, 11)
				BeginTextCommandSetBlipName "STRING"
				AddTextComponentString(string.format("(Owned) %s", v.label))
				EndTextCommandSetBlipName(HouseBlip)
				_AllHousesBlips[k] = HouseBlip
			else
				local HouseBlip = AddBlipForCoord(coords.front.x, coords.front.y, coords.front.z)
				...
				SetBlipCategory(HouseBlip, 11)
				BeginTextCommandSetBlipName "STRING"
				AddTextComponentString(string.format("(Avail) %s", v.label))
				EndTextCommandSetBlipName(HouseBlip)
				_AllHousesBlips[k] = HouseBlip
			end
		end
	else
		for k, v in pairs(_AllHousesBlips) do
			if next(_AllHousesBlips) ~= nil then
				RemoveBlip(_AllHousesBlips[k])
			end
		end
	end
end)
...
PROPERTIES = {
	...
	Furniture = {
		...
		Place = function(self, model, category, metadata, blockBrowse, skipPhone)
			...
			ObjectPlacer:Start(GetHashKey(model), "Furniture:Client:Place", metadata, true, "Furniture:Client:Cancel", true, true)

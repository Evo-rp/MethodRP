local PER_PAGE = 6
_MDT.Reports = {
	Search = function(self, term, type, tagsFilter, page)
		...
		Database.Game:aggregate({
			...
			p:resolve({
                ...
                pages = results[1]?.pagination[1]?.total or 1,
            })
        end, true)
...
AddEventHandler("MDT:Server:RegisterCallbacks", function()
    Callbacks:RegisterServerCallback("MDT:Search:report", function(source, data, cb)
		...
		if CheckMDTPermissions(source, false) or char:GetData("Attorney") then
			cb(MDT.Reports:Search(data.term, data.reportType, data.tags, data.page))
		else
			cb(false)
		end
    end)

...
import { Grid, TextField, ButtonGroup, Button, Backdrop, FormControlLabel, Checkbox, MenuItem } from '@mui/material';
			...
			<Grid container style={{ height: '100%', paddingBottom: 10 }} spacing={2}>
				<Grid item xs={12}>
					<Grid container className={classes.formActions}>
						<Grid item xs={10}>
							<div className={classes.title}>{Boolean(state?._id) ? `Edit Receipt` : `New Receipt`}</div>
						</Grid>
				...
				<Grid item xs={6} className={classes.col}>
					...
					>
						{recTypes
							.filter((r) => !r.jobs || r.jobs.includes(job?.Id))
							.map((option) => (

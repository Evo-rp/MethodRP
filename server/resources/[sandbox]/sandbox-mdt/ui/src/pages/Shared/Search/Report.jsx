...
export default () => {
	...
	useEffect(() => {
		if (qry.search) {
			...
			fetch(qry.search, false, [], qry.page);
		} else if (searchTerm && searchTerm != '') {
			fetch(searchTerm, reportType, tags, qry.page);
		}
	}, []);
	...
	const fetch = useMemo(() => throttle(async (value, rType, rTags, page) => {
		...
		try {
			...
			if (res?.data) {
				...
				const filteredResults = res.data?.filter(report => {
					if (isAttorney) {
	...
	const onSearch = (e) => {
		...
		setPage(1);
		fetch(searchTerm, reportType, tags, 1);
	};
	...
	const onPagi = (e, p) => {
		...
		setPage(p);
		fetch(searchTerm, reportType, tags, p);
	};

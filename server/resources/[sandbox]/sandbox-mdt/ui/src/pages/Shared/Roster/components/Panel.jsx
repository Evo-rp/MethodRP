...
export default ({ selectedJob, officer, onUpdate }) => {
	...
	const isSuspended = checkSuspension(officer.MDTSuspension, myJob.Id);
			...
			<Grid container style={{ height: '100%' }}>
				{(officer.SID != user.SID && !officer.MDTSystemAdmin) && <Grid item xs={12}>
					<ButtonGroup fullWidth>
						...
						{(canFire && !isSuspended && (myJob.Id === 'police' || myJob.Id === 'ems')) && <Button onClick={() => setJobSuspend(true)}>
							Suspend
						</Button>}
						{(canFire && isSuspended && (myJob.Id === 'police' || myJob.Id === 'ems')) && <Button onClick={() => setRevokingSuspend(true)}>
							Revoke Suspension
						</Button>}

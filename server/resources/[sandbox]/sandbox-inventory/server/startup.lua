...
function RegisterCommands()
	...
	Chat:RegisterAdminCommand("openinv", function(source, args, rawCommand)
		...
		local char = Fetch:SID(tonumber(args[1])):GetData("Character")
		if char == nil then
	...
	Chat:RegisterAdminCommand("clearinventory", function(source, args, rawCommand)
		local player = exports["sandbox-base"]:FetchComponent("Fetch"):SID(tonumber(args[1]))
		if player == nil then
	...
	Chat:RegisterAdminCommand("clearinventory2", function(source, args, rawCommand)
		...
		if Owner and Type then
			MySQL.query.await("DELETE FROM inventory WHERE name = ?", { string.format("%s-%s", Owner, Type) })
			Execute:Client(
				source,
				"Notification",
				"Success",

...
function RetrieveComponents()
	...
	EntityTypes = exports["sandbox-base"]:FetchComponent("EntityTypes")
		...
		Middleware:Add("Characters:Spawning", function(source)
			Citizen.CreateThread(function()
				for k, v in pairs(_cachedProps) do
					if k ~= source then
						TriggerClientEvent("Weapons:Client:UpdateProps", source, k, v)
					end
				end
			end)
		end, 2)
...
function SetGunPropData(source, sid, inv)
	local data = {}
	...
	TriggerClientEvent("Weapons:Client:UpdateProps", -1, source, data)
end
...
function DoSwap(source, data, cb)
	Citizen.CreateThread(function()
		...
		if totWeight > getCapacity(data.invTypeTo, data.vehClassTo, data.vehModelTo, data.capacityOverrideTo) and ((data.ownerFrom ~= data.ownerTo) or (data.invTypeFrom ~= data.invTypeTo)) then
			cb({ reason = "Inventory Over Weight" })
			sendRefreshForClient(source, data.ownerTo, data.invTypeTo, data.slotTo)
			sendRefreshForClient(source, data.ownerFrom, data.invTypeFrom, data.slotFrom)
			return
...
INVENTORY = {
	...
	CreateItem = function(self, Owner, Name, Count, Slot, md, invType, isRecurse, forceCreateDate, quality)
		...
		if itemExist then
			...
			elseif itemExist.name == "govid" and invType == 1 then
				...
				MetaData.Mugshot = char:GetData("Mugshot")
			elseif itemExist.name == "moneybag" and not MetaData.Finish then
				MetaData.Finished = os.time() + (60 * 60 * 24 * math.random(1, 3))
			elseif itemExist.name == "crypto_voucher" and not MetaData.CryptoCoin and not MetaData.Quantity then
				MetaData.CryptoCoin = "PLEB"
	...
	UpdateGovIDMugshot = function(self, stateId, mugshot)
		...
		local res = MySQL.query.await('SELECT id, information as MetaData FROM inventory WHERE item_id = ? AND JSON_EXTRACT(information, "$.StateID") = ?', { 
			"govid",
			stateId
		})
		...
		if #updatingIds > 0 then
			MySQL.query.await(string.format('UPDATE inventory SET information = JSON_SET(information, "$.Mugshot", ?) WHERE id IN (%s)', table.concat(updatingIds, ',')), {
				mugshot
			})
		end
		return true

...
CRAFTING = {
	...
	Craft = {
		Start = function(self, crafter, bench, result, qty)
			if _inProg[crafter] ~= nil or _types[bench] == nil or (_types[bench].recipes[result] == nil and not _types[bench].isPubSchemTable) then
				return { error = true, message = "Already Crafting" }
			end

			local recipies = _types[bench].recipes
			if _types[bench].isPubSchemTable then
				recipies = GetCharacterPublicSchemsRecipies(crafter, _types[bench].isPubSchemTable)
			end

			local reagents = {}
			for k, v in ipairs(recipies[result].items) do
			...
			local makingItem = recipies[result].result
			...
			if _types[bench].isPubSchemTable then
				...
				if _cooldowns[bench] and _cooldowns[bench][result] ~= nil and _cooldowns[bench][result] > (os.time() * 1000) then
			...
			local t = deepcopy(recipies[result])
			t.time = t.time * qty
		...
		End = function(self, crafter)
			...
			local recipies = _types[_inProg[crafter].bench].recipes
			if _types[_inProg[crafter].bench].isPubSchemTable then
				recipies = GetCharacterPublicSchemsRecipies(crafter, _types[_inProg[crafter].bench].isPubSchemTable)
			end

			local recipe = recipies[_inProg[crafter].result]
			...
			if recipe.cooldown then
				if _types[_inProg[crafter].bench].isPubSchemTable then
					InsertCooldown(string.format("%s:%s", _inProg[crafter].bench, crafter), recipe.id, (os.time() * 1000) + recipe.cooldown)
				else
					InsertCooldown(_inProg[crafter].bench, recipe.id, (os.time() * 1000) + recipe.cooldown)
				end
			end

			if _inProg[crafter] then
	...
	Schematics = {
		Has = function(self, bench, item, stateID)
			if _types[bench] ~= nil then
				if not _types[bench].isPubSchemTable then
					...
					local playerSchematics = GetCharacterPublicSchems(stateID)
					if playerSchematics and playerSchematics[_types[bench].isPubSchemTable] then
						for _, s in ipairs(playerSchematics[_types[bench].isPubSchemTable]) do
							if s == item then
								return true
							end
						end
		...
		Add = function(self, bench, item, stateID)
			if _types[bench] ~= nil and _schematics[item] ~= nil then
				if not Crafting.Schematics:Has(bench, item, stateID) then
					if _types[bench].isPubSchemTable then
						return AddCharacterPublicSchems(stateID, _types[bench].isPubSchemTable, item)
					else
						local p = promise.new()
...
function RegisterCraftingCallbacks()
	Callbacks:RegisterServerCallback("Crafting:GetBenchDetails", function(source, data, cb)
		...
		then
			...
			local cooldowns = _cooldowns[data]

			if bench.isPubSchemTable then
				recipies = GetCharacterPublicSchemsRecipies(char:GetData("SID"), bench.isPubSchemTable)
				cooldowns = _cooldowns[string.format("%s:%s", data, char:GetData("SID"))]
			end
		...
		if plyr ~= nil then
			...
			if char ~= nil then
				...
				for k, v in ipairs(schems) do
					...
					if itemData?.schematic ~= nil and not Crafting.Schematics:Has(data.id, v.Name, char:GetData("SID")) then
	...
	Middleware:Add("playerDropped", function(source)
		...
		if plyr ~= nil then
			local char = plyr:GetData("Character")
			if char ~= nil and _publicSchemData[stateID] then
				_publicSchemData[stateID] = nil
			end
		end
	end, 5)
...
function RegisterPublicSchematicBenches()
	for k, v in ipairs(PublicSchematicTables) do
		Crafting:RegisterBench(
			string.format("public-schematic-crafting-%s", v.id),
			"Use Bench",
			v.targetConfig,
			v.location,
			{ shared = true },
...
function GetCharacterPublicSchems(stateID, forceRefresh)
	if _publicSchemData[stateID] and not forceRefresh then
		return _publicSchemData[stateID]
	end

	local p = promise.new()
	Database.Game:findOne({
		collection = "schematics_public_bench",
		query = {
			owner = stateID
		},
	}, function(success, results)
		if success and results[1]?.schematics then
			p:resolve(results[1].schematics)
		else
			p:resolve({})
		end
	end)
	...
	local res = Citizen.Await(p)
	_publicSchemData[stateID] = res

	return _publicSchemData[stateID]
end

function AddCharacterPublicSchems(stateID, benchID, schematic)
	local p = promise.new()
	Database.Game:updateOne({
		...
		update = {
			["$push"] = {
				[string.format("schematics.%s", benchID)] = schematic
			}
		},
		options = {
			upsert = true
	...
	if res and _publicSchemData[stateID] then
		if not _publicSchemData[stateID][benchID] then
			_publicSchemData[stateID][benchID] = {
				schematic
			}
		else
			local current = _publicSchemData[stateID][benchID]
			table.insert(current, schematic)

			_publicSchemData[stateID][benchID] = current
		end
	end
...
function GetCharacterPublicSchemsRecipies(stateID, benchID)
	local res = {}
	local playerSchematics = GetCharacterPublicSchems(stateID)

	if playerSchematics and playerSchematics[benchID] then
		for _, s in ipairs(playerSchematics[benchID]) do
			local f = table.copy(_schematics[s] or {})
			f.schematic = s

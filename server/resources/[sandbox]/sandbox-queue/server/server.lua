...
_QUEUE = {
	...
	HandleConnection = function(self, source, identifier, deferrals)
		...
		QUEUE_PLAYERS[identifier] = ply:GetQueueLevel() + _queueIndex
		...
		while GetPlayerEndpoint(source) and not queueClosed and (
			((not Queue.Queue:Check(identifier)) or (CURRENT_CONNECTORS_COUNT >= Config.Settings.AllowedPerTick)) 
		...
		if queueClosed then
			...
			return CancelEvent(Config.Strings.PendingRestart)
		end
	...
	Queue = {
		...
		Check = function(self, identifier)
			...
			-- local openSlots = (MAX_PLAYERS - GetNumPlayerIndices()) - CURRENT_CONNECTORS_COUNT
			-- local count = 1
			-- for i = 1, openSlots do
			-- 	if QUEUE_PLAYERS_SORTED[i] == identifier then
			-- 		return true
			-- 	end
			-- end
			return false
		...
		CheckGhosts = function(self)
			for k, v in pairs(CURRENT_CONNECTORS) do
				if v and not GetPlayerEndpoint(v.source) then
					...
					print("Force Removed From Connectors List!", v.source)
				end
			end

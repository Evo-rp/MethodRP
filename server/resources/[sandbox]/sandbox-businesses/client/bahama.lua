...
AddEventHandler('Businesses:Client:Startup', function()
    ...
    Interaction:RegisterMenu("bh_stripper_pole", "Bahama Mamas Dancers", "party-horn", function()
        if (Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_dancers_1') or Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_dancers_2') or Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_dancers_3')) and LocalPlayer.state.onDuty == 'bahama' and Jobs.Permissions:HasPermissionInJob('bahama', 'STRIP_POLE') then
            local subMenu = {}
        ...
        return (Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_dancers_1') or Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_dancers_2') or Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_dancers_3')) and LocalPlayer.state.onDuty == 'bahama'
    end)
    ...
    Interaction:RegisterMenu("bh_makeitrain", 'Make It Rain', "money-bill-1-wave", function()
        if not _makingItRain and Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_makeitrain') then
            local makeItRain = {
                ...
                },
                {
                    type = 'moneyroll',
                    text = 'Money Rolls',
                    time = math.random(12000, 15000),
                },
                {
                    type = 'moneyband',
                    text = 'Money Bands',
                    time = math.random(15000, 20000),
                }
            }
        ...
        return Polyzone:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'bh_makeitrain') and GetNearbyFuckingStripper()
    end)
end)

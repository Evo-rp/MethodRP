window.APP = {
  ...
  mounted() {
    post('https://sandbox-chat/loaded', JSON.stringify({}));
  ...
  methods: {
    ...
    send(e) {
      if(this.message !== '') {
        post('https://sandbox-chat/chatResult', JSON.stringify({
    ...
    autoComplete() {
      if (this.topSuggestion) {
        this.message = this.topSuggestion;
      }
    },
    suggestionsChange(data) {
      if (data.length > 0 && !data[0].disabled) {
        this.topSuggestion = data[0].name;
      } else {
        this.topSuggestion = false;
      }
    },
    hideInput(canceled = false) {
      if (canceled) {
        post('https://sandbox-chat/chatResult', JSON.stringify({ canceled }));
      }
      this.message = '';

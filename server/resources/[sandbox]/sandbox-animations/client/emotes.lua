...
function DoAnEmote(emoteData, fromUserInput, length, notCancellable, emoteName, skipDisarm)
    ...
    local isK9 = LocalPlayer.state.isK9Ped

    if (fromUserInput and emoteData.AdditionalOptions.AvailableToK9 and isK9) or (fromUserInput and emoteData.AdditionalOptions.AvailableToChar and not (emoteData.AdditionalOptions.AvailableToK9 and not isK9)) or (not fromUserInput) or LocalPlayer.state.isAdmin then
        ...
        if ChosenDict and ChosenDict ~= "" then
            LoadAnim(ChosenDict)
        end

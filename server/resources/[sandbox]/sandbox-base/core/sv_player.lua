...
AddEventHandler("Proxy:Shared:RegisterReady", function()
	COMPONENTS.Middleware:Add("playerDropped", function(source, message)
		...
		if player ~= nil then
			...
			if player:GetData("LastLocation") and type(player:GetData("LastLocation")) == "vector3" then
				lastLocationMessage = string.format(" [Coords: %s]", player:GetData("LastLocation"))
			end

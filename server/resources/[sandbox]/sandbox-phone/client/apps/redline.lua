...
local function UnGhostPlayer()
	...
	if not NotiySent and ForceStop == false then
		NotiySent = true
		Notification:Standard("Phasing: Disabled")
...
local function GhostPlayer()
	...
	if not NotiySent then
		NotiySent = true
		Notification:Standard("Phasing: Enabled")
		Citizen.SetTimeout(2000, function()
			NotiySent = false
		end)
	end
...
function StartRace()
	...
	Citizen.CreateThread(function()
		...
		while _activeRace ~= nil and _loggedIn do
			...
			-- if cp ~= nil and next(cp) ~= nil and dist > 150 and _activeRace.phasing then
			-- 	UnGhostPlayer()
			-- 	ForceStop = true
			-- else

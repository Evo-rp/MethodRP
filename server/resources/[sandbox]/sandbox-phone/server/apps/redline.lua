...
function FinishRace(id)
	...
	if _races[id].completed ~= nil and #_races[id].completed > 0 then
		...
		_races[id].competitive = false
		if
			(_races[id].total >= _reqForCrypto)
			and (_races[id].trackData.Type == "p2p" or tonumber(_races[id].laps) > 1)
		then
			_races[id].competitive = true
		end
...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
	...
	Callbacks:RegisterServerCallback("Phone:Redline:StartRace", function(src, data, cb)
		...
		then
			...
			if
				_races[tonumber(data)].class ~= "All"
				and CheckVehicleAgainstClass(_races[tonumber(data)].class, char:GetData("Source")) == false
			then
				Phone.Notification:Add(
					char:GetData("Source"),
					"Unable to Join Race",
				...
				if dist > 25 then
					...
					Robbery:TriggerPDAlert(
						...
						"10-94",
						string.format("Illegal Street Racing [%s]", _races[tonumber(data)].class),
						{
							icon = 227,
							size = 0.9,
							color = 31,

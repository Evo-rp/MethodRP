...
import { useAlert, useCompanyUpgrades, useJobPermissions } from '../../hooks';
		...
		try {
			...
			if (res.success) {
				...
				switch (res.code) {
					...
					case 'ERROR':
						sendAlert(
							`Unable To ${grade.edit ? 'Update' : 'Create'
							} Grade`,
						);
						break;
					case 'INVALID_PERMISSIONS':
													...
													return (
														...
														>
															<ListItemText
																...
																secondary={`${Object.keys(grade.Permissions).filter(perm => grade.Permissions[perm]).length
																	} Permissions`}
															/>
														</ListItem>
													);
							...
							>
								...
								>
									{Object.keys(jobPermissions).filter(p => {
										...
										return (
											<MenuItem key={`perm-${key}`} value={key} disabled={!hasPerm(key, jobData?.Id) && !isOwner}>
